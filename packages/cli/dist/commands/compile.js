...
console.fatal = function (message, code = 1) {
    console.log(message);
    process.exit(code);
};
    ...
    if (args.default && args.default.length > 0) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            if (fs.existsSync(outPath) && !args.force) {
                console.fatal("Output file already exists, add the --force flag to force removal of the original.");
            }
            compileContent(content, Object.assign(Object.assign({ __FILE__: filePath, __DIR__: path.dirname(filePath) }, DEFAULT_CONSTANTS), DEFAULT_METHODS)).then((result) => {
        ...
        if (!fs.existsSync(config)) {
            console.fatal("The configuration file could not be found in the specified location. Use --config to set the configuration file path.");
        }
        ...
        if (rootDir && outDir) {
            ...
            for (const file of files) {
                ...
                if (file.match(/\.tess/g) && fs.existsSync(file)) {
                    ...
                    compileContent(fileContent, Object.assign(Object.assign({ __FILE__: file, __DIR__: path.dirname(file) }, DEFAULT_CONSTANTS), DEFAULT_METHODS), {}, {
                        defaultLanguage
                    }).then((result) => {
...
async function compileContent(content, variables, environment = {}, options = {}) {
    const converter = new Tess(options);
    converter.compile(content);
    // Get the output
    environment = Object.assign(Object.assign(Object.assign({}, environment), variables), { Markdown: new MarkdownExtension(content) });
