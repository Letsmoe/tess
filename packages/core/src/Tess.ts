...
class Tess {
	...
	private oldBuffer = [];
	private opsStack = [];
	private _render_function: Function;
	/**
	 *
		...
		for (const token of tokens) {
			if (token.startsWith("{!")) {
				...
				if (this.environment.hasOwnProperty(tagName)) {
					let beginString: string, kwargs: Kwargs | string, args: Args;
				...
				if (words[0] === "code") {
					...
					const { kwargs, args } = this.parseArguments(
						words.slice(1).join(" ")
					);
				...
				if (endWhat == "code") {
					if (startWhat.kwargs.var) {
						...
						code.addLine(
							`let ${startWhat.kwargs.var} = await _execute_code("${startWhat.kwargs.lang}", [`
						);
					} else {
						code.addLine(
							`await _execute_code("${startWhat.kwargs.lang}", [`
						);
					}
	...
	public async render(context = null) {
		...
		return await this._render_function(
			context,
